...
def checkNLogUser():
    global stuName
    stuName=""
    user = lite.connect('user.db')
    c = user.cursor()
    username = request.form['username']
    password = request.form['password']
    print(username , password)
    c.execute('SELECT * FROM UserDetail WHERE ( Username == "{n1}") ORDER BY RANDOM() LIMIT 1'.\
        format( n1 = username ))
...
def signUserUp():
    ...
    if(password==password0):
        with user:
            ...
            c1.execute("INSERT INTO UserDetail(Name, Gender, Email, Username, Password) VALUES(?,?,?,?,?)",(name,gender,email,username,password))
        user.close()
        return redirect(url_for('login'))
    else:
